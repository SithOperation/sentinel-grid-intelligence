...
def create_conflict_event(article):
    ...
    event = create_event(
        ...
        priority="high",
        title=article.get("title", "Conflict Intelligence Report"),
    ...
    event["verification"] = {"confirmed": False, "source_count": 1}
    ...
    try:
        ...
        for article in articles:
            ...
            if is_conflict_related(text):
                event = create_conflict_event(article)
        ...
        for source, count in source_counts.items():
            print(f"[+] {source} conflict events: {count}")

    except Exception as error:
        print("[!] Conflict collector failed:", error)
