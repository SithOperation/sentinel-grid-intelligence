...
def create_maritime_event(message):
    ...
    position = message.get("Message", {})

    ship = position.get("PositionReport", {})
...
def collect_maritime():
    ...
    try:
        ...
        for message in messages:
            events.append(create_maritime_event(message))

        print(f"[+] AISStream collected {len(events)} vessel events")

    except Exception as error:
        print("[!] Maritime collector failed:", error)
