...
def create_aircraft_event(aircraft):
    ...
    event["aircraft"] = {
        "icao": safe_value(aircraft[0]),
        "callsign": safe_value(aircraft[1].strip() if aircraft[1] else None),
        "origin_country": safe_value(aircraft[2]),
        "altitude": safe_value(aircraft[7], 0),
        "velocity": safe_value(aircraft[9], 0),
        "heading": safe_value(aircraft[10], 0),
    }

    observation_time = aircraft[3] or aircraft[4]
    if observation_time:
        event["timestamp"] = datetime.fromtimestamp(
            observation_time, timezone.utc
        ).isoformat()
...
def collect_aircraft():
    ...
    try:
        ...
        for aircraft in aircraft_list[:MAX_AIRCRAFT]:
            ...
            events.append(create_aircraft_event(aircraft))

        print(f"[+] OpenSky aircraft collected {len(events)} events")

    except Exception as error:
        print("[!] OpenSky aircraft collector failed:", error)
