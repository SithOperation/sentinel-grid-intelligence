...
def geolocate_event(event):
    if event.get("event_type") == "x_report":
        # Official X place coordinates are already validated by the collector,
        # including legitimate points on the equator or prime meridian.
        return event
...
def apply_geolocation(events):
    ...
    for event in events:
        processed.append(geolocate_event(event))
